...
def parse_arguments():
    parser = argparse.ArgumentParser(description='HGI region prediction (k-fold, single city, original mode only)')
...
def load_embeddings(city, task):
    abbr = city_abbreviation(city)
    pkl_path = get_suburban_dir('embs', 'HGI', city, f'{task}_{abbr}_HGI.pkl')
