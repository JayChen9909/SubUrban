...
class POIEncoder(nn.Module):
    ...
    def forward(self, x, edge_index, edge_weight):
        assert torch.all(edge_weight > 1e-8), "Edge weights contain zero or near-zero values"
...
class HierarchicalGraphInfomax(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        for region in range(torch.max(data.region_id)+1):
            ...
            # Ban this part of using hard negative sampling with input of coarse_region_similarity
            if hard_negative_choice < 0:
