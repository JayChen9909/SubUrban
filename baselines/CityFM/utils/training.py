...
def serialize_data(data, tok, add_zero=False):
    ...
    for d in data:
        ...
        entities_tags.append(s_tags) # "name:肯德基 address:北京"
...
def train_SS_im(cd, model):
    ...
    for epoch in range(config.n_epochs):
        ...
        batch_size = config.index

        real_range = len(queries) - config.index
        for idx in tqdm(range(0, real_range, batch_size), desc='Batched training'):
        ...
        #     else:
            start = idx
            end = min(idx + batch_size, real_range)
