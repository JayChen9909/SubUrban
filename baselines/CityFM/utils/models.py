...
def PE_new(pos, dim):
    ...
    if not isinstance(pos, torch.Tensor):
        pos = torch.tensor(pos, dtype=torch.float32, device='cuda')

    num_pois = pos.shape[0]
    device = pos.device

    # Compute w_k_values
    i_values = torch.arange(0, dim // 2, 2, device=device)  # Shape: [dim // 4]
    exponents = i_values / (dim // 2)
    w_k_values = 100 / torch.pow(10000, exponents)  # Shape: [dim // 4]

    # Expand dimensions for broadcasting
    w_k_expanded = w_k_values.view(1, -1, 1)  # Shape: [1, dim // 4, 1]
    pos_expanded = pos.unsqueeze(1)  # Shape: [num_pois, 1, 2]

    # Compute loc * w_k
    loc_times_wk = pos_expanded * w_k_expanded  # Shape: [num_pois, dim // 4, 2]

    # Compute sin and cos
    sin_values = 100 * torch.sin(loc_times_wk)  # Shape: [num_pois, dim // 4, 2]
    cos_values = 100 * torch.cos(loc_times_wk)  # Shape: [num_pois, dim // 4, 2]

    # Reshape and interleave sine and cosine values
    sin_cos = torch.stack((sin_values, cos_values), dim=3)  # Shape: [num_pois, dim // 4, 2, 2]
    sin_cos = sin_cos.view(num_pois, -1)  # Shape: [num_pois, dim]
...
class InfoNCE(nn.Module):
    ...
    def __init__(self, batch_size, temperature=0.5):
        ...
        self.mask = (~torch.eye(batch_size * 2, batch_size * 2, dtype=bool)).float().cuda()
